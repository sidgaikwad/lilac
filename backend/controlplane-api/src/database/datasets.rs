...
impl Database {
    pub async fn get_dataset(&self, dataset_id: &DatasetId) -> Result<Dataset, DatabaseError> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| translate_sqlx_error(String::from("dataset"), dataset_id.to_string(), e))?;
        Ok(dataset)
    }
    ...
    pub async fn create_dataset(&self, dataset: Dataset) -> Result<DatasetId, DatabaseError> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| translate_sqlx_error(String::from("dataset"), dataset.dataset_id.to_string(), e))?;
        Ok(dataset_id)
    }

    pub async fn delete_dataset(&self, dataset_id: &DatasetId) -> Result<(), DatabaseError> {
        let id = dataset_id.inner();
        sqlx::query!(
            // language=PostgreSQL
            r#"
