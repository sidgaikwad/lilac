...
const mockDataSetDetail = {
  ...
  originalDataPreview: { type: 'csv', content: 'CustomerID,Gender,SeniorCitizen,Partner,Dependents,tenure...\n1234,Male,0,Yes,No,1...' },
  pipelineRuns: [
    {
      runId: 'run-abc',
      ...
      derivedDataSet: {
        ...
        preview: { type: 'csv', content: 'CustomerID,Gender,SeniorCitizen,Partner,Dependents,TenureMonths...\n1234,Male,0,Yes,No,1...' },
        subRuns: [
           {
             runId: 'run-def',
             ...
             derivedDataSet: {
               ...
               preview: { type: 'csv', content: 'CustomerID,IsMale,IsSenior,HasPartner,HasDependents,TenureCategory...\n1234,1,0,1,0,New...' },
    ...
    },
    {
      ...
      derivedDataSet: {
        ...
        preview: { type: 'image_folder', content: 'Preview of image thumbnails...' },
        subRuns: []
      }
    }
  ]
...
const DerivedDataSetNode: React.FC<DerivedDataSetNodeProps> = ({ runId, pipelineName, timestamp, derivedDataSet, level = 0 }) => {
  const [isOpen, setIsOpen] = useState(true); // Default to open for visibility

  const handleToggle = () => setIsOpen(!isOpen);
  const handleViewData = () => {
      alert(`Mock: Viewing data for ${derivedDataSet.name} (ID: ${derivedDataSet.id})`);
      // TODO: Implement actual data viewing logic (e.g., open a modal, navigate)
  }

  return (
    <div style={{ marginLeft: `${level * 20}px` }} className="mt-2 border-l pl-3">
       <div className="flex items-center cursor-pointer" onClick={handleToggle}>
         ...
         <span className="text-xs text-muted-foreground ml-2">(Run: {pipelineName} @ {timestamp})</span>
         <Button variant="ghost" size="sm" className="ml-auto" onClick={(e) => { e.stopPropagation(); handleViewData(); }}>View Data</Button>
       </div>
       {isOpen && derivedDataSet.subRuns.map(subRun => (
         <DerivedDataSetNode key={subRun.runId} {...subRun} level={level + 1} />
       ))}
       {isOpen && derivedDataSet.subRuns.length === 0 && (
          <div style={{ marginLeft: `${(level + 1) * 20}px` }} className="mt-1 text-sm text-muted-foreground pl-4">
...
const DataSetDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>(); // Get dataset ID from URL
    ...
    <div className="container mx-auto p-4 md:p-6 lg:p-8">
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>
            Source: {dataSet.source} | Created: {dataSet.createdAt} | ID: {dataSet.id}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <h3 className="text-lg font-semibold mb-2">Original Data</h3>
           <div className="flex items-center p-3 border rounded bg-gray-50 dark:bg-gray-800">
             ...
             <div className="flex-grow">
                ...
                <p className="text-xs text-muted-foreground truncate">Preview: {dataSet.originalDataPreview.content}</p>
             </div>
             <Button variant="outline" size="sm" onClick={handleViewOriginalData}>View Full Data</Button>
           </div>
        </CardContent>
      </Card>

      <h2 className="text-xl font-semibold mb-4">Pipeline Runs & Derived Data Sets</h2>
      {dataSet.pipelineRuns.length > 0 ? (
        dataSet.pipelineRuns.map(run => (
          <DerivedDataSetNode key={run.runId} {...run} />
        ))
      ) : (
        <p className="text-muted-foreground">No pipeline runs have been performed using this dataset yet.</p>
      )}
    </div>
  );
};
