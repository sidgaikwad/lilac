...
const sectionLinks: Record<string, { title: string; items: { href: string; label: string }[] }> = {
    ...
    datasets: { // Ensure 'datasets' section exists
        title: "Data Sets",
        items: [
...
const ProjectSectionSidebar: React.FC<ProjectSectionSidebarProps> = ({ activeSection = 'pipelines' /* Default to pipelines for now */ }) => {
    ...
    return (
        <aside className="w-64 bg-background border-r border-border p-4 shrink-0 hidden md:block"> {/* Hide on small screens */}
            <ScrollArea className="h-full">
                <h2 className="text-lg font-semibold mb-4 px-4">{currentSection.title}</h2>
                <nav className="flex flex-col gap-1">
                    {currentSection.items.map(item => (
                        <NavLink
                            ...
                            to={`/projects/${projectId}/${activeSection}/${item.href}`.replace(/\/$/, '')} // Remove trailing slash if href is empty
