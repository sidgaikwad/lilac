...
mod client;
use client::{ApiClient, SubmitJobRequest, ResourceRequirements, GpuRequirement};
...
async fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        Commands::Submit {} => {
            ...
            if gpu_required {
                gpu_count = Some(Input::with_theme(&theme)
                    .with_prompt("How many GPUs?")
                    .interact_text()?);

                gpu_model = Some(Input::with_theme(&theme)
            ...
            println!("- Queue: {} ({})", selected_queue.name, selected_queue.id);
            println!("- CPU: {}m", cpu);
            ...
            let image_tag: String = rand::rng()
                ...
                .map(char::from)
                .collect();
            let image_name = format!("{}/{}:{}", config.container_registry_url, name, image_tag);
...
"#,
            ...
            fs::write(build_context.path().join("Dockerfile"), dockerfile_content)?;
