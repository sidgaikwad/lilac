...
impl HybridMonitor {
    ...
    fn get_cpu_millicores() -> anyhow::Result<i32> {
        if Path::new("/sys/fs/cgroup/cpu.max").exists() {
            let cpu_max = fs::read_to_string("/sys/fs/cgroup/cpu.max")?.trim().to_string();
            let parts: Vec<&str> = cpu_max.split_whitespace().collect();
            if parts.len() == 2 && parts[0] != "max" {
                let max_us: f64 = parts[0].parse()?;
                let period_us: f64 = parts[1].parse()?;
    ...
    fn get_memory_mb() -> anyhow::Result<i32> {
        if Path::new("/sys/fs/cgroup/memory.max").exists() {
            let mem_max_bytes: i64 = fs::read_to_string("/sys/fs/cgroup/memory.max")?.trim().parse()?;
